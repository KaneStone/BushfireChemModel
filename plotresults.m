...
d.doublelinear2 = load([inputs.outputdir,'runoutput/','doublelinear','_',num2str(inputs.altitude),...
    'km_',abs(num2str(inputs.latitude)),inputs.hemisphere...
    ,'_',num2str(inputs.fluxcorrections),'flux_',sprintf('%.2f',inputs.hourstep),'hours_aheffectsall_visx1e5.mat']);
...
%     coldmodelCLONO2latextract = weightedaverage(squeeze(coldvertical.CLONO2.regrid(oldlatind,lev,:)),oldlatextract ,1);
    ...
    for i = 1:length(vars2)
        ...
        lh = legend([ph1,ph2,ph3,phb],'CARMA control','CARMA solubility',...
            ...
            'Box model double linear visx10000',...        
            'Box model double linear ah hcl and limited hcl solx4',...        
    ...
    savefigure([inputs.outputdir,'figures/'],[vars2{1},'_CARMA_reactions_ahaffectsall_vis1e5_comparison'],1,0,0,0);
    a = 1;
    %set(gca,'Yscale','log')
